...
static HANDLE garmin_usb_start(HDEVINFO hdevinfo, SP_DEVICE_INTERFACE_DATA *infodata)
{
	DWORD size;
...
gusb_init(const char *pname, gpsdevh **dh)
{
    ...
    winusb_unit_data *wud = reinterpret_cast<winusb_unit_data*>(xcalloc(sizeof (winusb_unit_data), 1));
	*dh = (gpsdevh*) wud;
